...
window.handleWithdraw = async function() {
    ...
    try {
        ...
        if (requestTx.status) {
            showAlert('Withdrawal request submitted successfully. You can complete the withdrawal after 1 hour.', 'success');
            document.getElementById('withdrawAmount').value = '';
            await updateUI();
            showPendingWithdrawal(amountWei);
        }
    } catch (error) {
        handleTransactionError(error);
    } finally {
...
// 出金処理 - Step 2: 出金実行
window.completeWithdrawal = async function() {
    if (!contract) return;
    ...
    try {
        ...
        showAlert('Completing withdrawal...', 'info');
...
// 出金リクエストのキャンセル
window.cancelWithdrawal = async function() {
    if (!contract) return;
    ...
    try {
        ...
        showAlert('Cancelling withdrawal request...', 'info');
        const cancelTx = await contract.methods.cancelWithdrawal()
            .send({ 
                from: currentAccount,
                gasPrice: gasPrice
            });
...
// 保留中の出金表示
function showPendingWithdrawal(amount) {
    const pendingWithdrawalDiv = document.getElementById('pendingWithdrawal');
    if (pendingWithdrawalDiv) {
        const amountInUSDC = web3.utils.fromWei(amount, 'mwei');
        pendingWithdrawalDiv.innerHTML = `
            <div class="alert alert-info">
                Pending withdrawal: ${amountInUSDC} USDC
                <button onclick="completeWithdrawal()" class="btn btn-primary">Complete Withdrawal</button>
                <button onclick="cancelWithdrawal()" class="btn btn-secondary">Cancel</button>
            </div>
        `;
        pendingWithdrawalDiv.style.display = 'block';
    }
}

// 保留中の出金非表示
function hidePendingWithdrawal() {
    const pendingWithdrawalDiv = document.getElementById('pendingWithdrawal');
    if (pendingWithdrawalDiv) {
        pendingWithdrawalDiv.style.display = 'none';
    }
}
